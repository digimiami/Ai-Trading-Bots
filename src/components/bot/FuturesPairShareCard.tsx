...
interface FuturesPairShareCardProps {
  pair: FuturesPair;
  suggestedSettings?: SuggestedBotSettings;
  isOpen: boolean;
  onClose: () => void;
}
...
export default function FuturesPairShareCard({ pair, suggestedSettings, isOpen, onClose }: FuturesPairShareCardProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      canvas.toBlob((blob) => {
        ...
        link.href = url;
        link.download = `pablobots-${pair.symbol}-${pair.exchange}-share.png`;
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        if (navigator.share && navigator.canShare) {
          const file = new File([blob], `pablobots-${pair.symbol}-share.png`, { type: 'image/png' });
          
          if (navigator.canShare({ files: [file] })) {
            try {
              await navigator.share({
                files: [file],
                title: `Check out ${pair.symbol} on PabloBots!`,
                text: 'Add to Active this Bot on PabloBots.com',
              });
              return;
            } catch (error: any) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" onClick={onClose}>
      <div className="bg-gray-900 rounded-lg max-w-md w-full p-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-white">Share Pair Card</h2>
          <button
            onClick={onClose}
        ...
        <div ref={cardRef} className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-xl p-5 text-white shadow-2xl relative overflow-hidden" style={{ width: '400px' }}>
          ...
          <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg p-3 mb-4 relative z-10 border border-gray-700/50">
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-sm font-bold text-white">{pair.symbol}</h4>
              <span className="px-2 py-1 bg-blue-600 text-white text-xs font-medium rounded uppercase">
                {pair.exchange}
              </span>
            </div>
