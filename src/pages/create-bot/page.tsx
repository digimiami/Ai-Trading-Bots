...
export default function CreateBotPage() {
  ...
  const urlExchange = (urlExchangeParam === 'bybit' ? 'bybit' : 'bybit') as 'bybit';
        ...
        try {
          ...
          if (data) {
            ...
            setFormData(prev => {
              ...
              const finalExchange = exchangeParam === 'bybit' ? 'bybit' : 'bybit';
              return {
                ...prev,
            ...
            if (parsedConfig && Object.keys(parsedConfig).length > 0) {
              setAdvancedConfig({ ...DEFAULT_ADVANCED_STRATEGY_CONFIG, ...parsedConfig } as AdvancedStrategyConfig);
            }
          }
        } catch (error: any) {
  ...
  const [advancedConfig, setAdvancedConfig] = useState<AdvancedStrategyConfig>(
    isFromBacktest && backtestData?.backtestAdvancedConfig
      ...
      : {
          ...DEFAULT_ADVANCED_STRATEGY_CONFIG,
          bias_mode: ((urlBiasMode as any) || DEFAULT_ADVANCED_STRATEGY_CONFIG.bias_mode) as AdvancedStrategyConfig['bias_mode'],
          regime_mode: ((urlRegimeMode as any) || DEFAULT_ADVANCED_STRATEGY_CONFIG.regime_mode) as AdvancedStrategyConfig['regime_mode']
        }
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.exchange !== 'bybit') {
        setError('Only Bybit is supported. Please use Bybit.');
        setIsCreating(false);
        return;
      }
            ...
            <Card className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center">
                    Exchange
                    <HelpTooltip text="Trading is supported on Bybit only." />
                  </label>
                  <select
                    value="bybit"
                    readOnly
