-- Migration: Set default strategy_config for new bots to allow shorts
-- This ensures all new bots allow both long and short trading by default
...
BEGIN
  ...
  IF NEW.strategy_config IS NULL THEN
    config_jsonb := '{}'::jsonb;
  ELSIF jsonb_typeof(NEW.strategy_config) IS NULL THEN
    -- If it's not a valid JSONB type, try to parse it as JSON
    BEGIN
      config_jsonb := NEW.strategy_config::text::jsonb;
    EXCEPTION WHEN OTHERS THEN
      -- If parsing fails, treat as empty object
      config_jsonb := '{}'::jsonb;
    END;
  ELSE
    config_jsonb := NEW.strategy_config;
  END IF;
...
DROP TRIGGER IF EXISTS trigger_set_default_strategy_config ON trading_bots;

-- Create trigger that runs before insert
CREATE TRIGGER trigger_set_default_strategy_config
  BEFORE INSERT ON trading_bots
  FOR EACH ROW
  EXECUTE FUNCTION set_default_strategy_config();

-- Add comment
COMMENT ON FUNCTION set_default_strategy_config() IS 'Sets default strategy_config for new bots to allow both long and short trading. Ensures bias_mode=auto and require_price_vs_trend=any for all new bots.';
