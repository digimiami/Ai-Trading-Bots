...
SELECT 
    ...
    CASE 
        WHEN tb.trading_type = 'paper' AND (ak.is_testnet IS NULL OR ak.is_testnet = false) 
            THEN '❌ MISMATCH: Paper bot using mainnet API key'
        WHEN tb.trading_type = 'real' AND ak.is_testnet = true 
            THEN '❌ MISMATCH: Real bot using testnet API key'
        WHEN tb.trading_type = 'paper' AND ak.is_testnet = true 
            THEN '✅ Correct: Paper bot with testnet key'
        WHEN tb.trading_type = 'real' AND (ak.is_testnet IS NULL OR ak.is_testnet = false) 
...
ORDER BY 
    CASE 
        WHEN tb.trading_type = 'paper' AND (ak.is_testnet IS NULL OR ak.is_testnet = false) THEN 1
        WHEN tb.trading_type = 'real' AND ak.is_testnet = true THEN 2
        ELSE 3
    END,
    tb.name;
