-- IMPORTANT: Run this SQL in Supabase SQL Editor to fix strategy_config validation
-- This allows partial updates (e.g., just updating max_trades_per_day) without requiring all fields

-- Drop existing constraint
ALTER TABLE trading_bots 
DROP CONSTRAINT IF EXISTS check_valid_strategy_config;

-- Recreate validation function with NULL checks (only validates fields that exist)
...
-- Success message
SELECT 'Strategy config validation fixed! Partial updates now allowed.' as status;
...
-- Success message
SELECT 'Paper trading system migration completed successfully!' as status;
